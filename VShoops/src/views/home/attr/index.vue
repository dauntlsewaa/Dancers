<template>
  ...
  <div class="notice">
   <div class="titles"> 
    ...
    v-for="(item,index) in data"
     :key="index" :class="{active:red==index}">
     {{item.name}}
    </span>
  </div>
   <el-button bg class="filtrate" @click="change=!change">筛选 <el-icon style="width: 16px;" :class="{icon:change}"><ArrowDown /></el-icon></el-button>
  ...
  <div style="padding: 26px 0;   position: relative;" v-if="change">
    ...
    <div class="interest" style="display:flex ;" >
      ...
      <div class="dance">
        <div class="danceitem " @click="btsHandler(int)" :class="{special:int.bt}" v-for="(int,index) in interest" :key="index">{{int.name}}</div>
      </div>
    </div>
<!-- int.bt=!int.bt -->
  <!-- 兴趣技能 -->
    <div class="interest" style="display:flex ;" >
      ...
      <div class="dance">
        <div class="danceitem"  @click="item.bt=!item.bt" :class="{special:item.bt}" v-for="(item,index) in formats" :key="index">{{item.name}}</div>
      </div>
    </div>
